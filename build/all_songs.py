songs = [ ( ("01_originaux","Morceaux originaux"), 
            [ ("01_prog_to_log_2000","prog","Prog-to-log 2000"),
              ("02_balade","balade", "Balade"),
              ("03_pour_juliette","pour_juliette","Pour Juliette (Partie II / Partie I)"),
              ("04_samba_lek","samba_lek","Samba Lek'"),
              ("05_valse_pour_zelie","pour_zelie", "Valse pour Zélie"),
              ("06_wuhan_connect","wuhan_connect","Wuhan connect"),
              ("07_disco","","Disco"),
              ("08_groove","","Groove"),
              ("09_turprendrasbiendlabonnesoupe","","Tu r'prendras bien de la bonne soupe ?"),
              ("10_agent000","agent000","Agent 000"),  
              ("11_rencontre_du_3eme_type","3emetype","Rencontre du 3ème type"),
              ("12_kronembourgvirus","kronembourgvirus", "Kronembourg-Virus Valse"),
              ("13_samba_calorea","samba_calorea","Samba Calor-é-a"),
              ("14_la_demoiselle_de_montreal","pour_perrine","La demoiselle de Montréal"),
              ("15_lalacestca","lalacestca","La la(ïcite), c'est ça !"),
              ("16_marche","marche","Marche"),
              ("17_c_est_pas_une_punition_han_2","punition","C'est pas une punition-han"),
              ("18_nouveau_morceau","nouveau","Nouveau morceau"),
              ("19_light_inside_the_body","light","Light inside the body"),
              ("20_rdv_cascade_pisse","cascade_pisse","Rendez-vous à la cascade de la pisse"),
              ("21_12+2","","12+2"),
              ("22_soup_soup","","Soup' soup'"),
              ("23_wuhan_disconnect","","Wuhan disconnect"),
              ("24_7_4_repetitif","","7/4 répétitif"),
              ("25_lemeilleurestavenir","lemeilleur","Le meilleur est à venir"),
              ("26_reverie","reverie","Rêverie"),
              ("27_larevanchedutriton","larevanchedutriton","La revanche du Triton"),
              ("28_quettehou","quettehou","Quettehou Monchâton ? Saint-Vaast ?"),
              ("29_volet","volet","Coucher de soleil (derrière un volet bloqué)"),
              ("30_bolerototo_newage","bolero","Boléro-toto, version New-Age-Olympique"),
              ("31_bolero","bolero","Boléro-toto"),
              ("32_bolero_jazz","bolero","Boléro-toto, version jazzy"),
              ("33_disco_tanz","","Disco Tanz"),
              ("34_pol_iss_pahr_tuh_just_iss_nuhl_pahr","","Pol Iss Pahr Tuh, Jüst Iss Nühl Pahr"),
              ("35_dimanche_mekanik","","Dimanche Mekanïk"),
              ("36_bouge_ton_corps","","Bouge ton corps"),
              ("37_frites","frites","Double ration de frites (c'est ça la République !)"),
              ("38_perdre_son_temps","perdresontemps","Perdre son temps"),
              ("39_prog_to_log_2023","prog","Prog-to-log 2023"),
              ("40_bolero_malgre_lui","boleromalgrelui","Boléro malgré lui"),
              ("41_liquide","liquide","Liquide"),
              ("42_ca_tourne","ca_tourne","Planète ça tourne"),
              ("43_chasse_a_l_homme","chasse_a_l_homme","Chasse à l'homme"),
              ("44_chasse_a_l_homme_instru","chasse_a_l_homme","Chasse à l'homme (instrumental)"),
              ("45_oudea_cassetoi","oudea_cassetoi","Ou-dé-a, Casse-toi-d'la !"),
              ("46_oudea_cassetoi_instru","oudea_cassetoi","Ou-dé-a, Casse-toi-d'la (instrumental)"),
              ("47_samedi_matin","samedi","Samedi matin"),
              ("48_provisoire","provisoire","Provisoire"),
              ("49_pour_manu","pour_manu","Pour Manu"),
              ("50_emotion_petit_bonhomme","emotion_du_petit_bonhomme","Emotion du petit bonhomme"),
              ("51_satirikiki","satirikiki","Satirikiki"),
              ("52_sur_un_air_de_jazz","sur_un_air_de_jazz","Sur un air de jazz"),
              ("53_remous","remous","Remous"),
              ("54_ponyonoke_2","ponyonoke","Ponyonoke dans le château sur la falaise"),
              ("55_descente_au_klezmeristan","descente","Descente au Klezmeristan"),
              ("56_5a7","5a7","Cinq à sept"),
              ("57_croisiere","croisiere","Croisière sur le Styx"),
              ("58_moerci_doerien","moerci_doerien","Moerci Doerien"),
              ("59b_operation_nikea","nikea","Opération Nikea"),
              ("61_solitude","","Solitude"),
              ("","mardi_prison","Mardi, prison de la santé"),
              ("","samba","Samba ni chaussettes"),
              ("","joie_bonheur","Joie & bonheur"),
              ("","blues","Blues"),
              ("","blues_confini","Le blues du con-fini"),
              ("","bras_casse","Bras cassé"),
              ("","cool_modul","Cool modulations"),
              ("","danse","Danse de (la fin de) l'abondance"),
              ("","ete_2001","Eté 2001"),
              ("","face","Dans ta F.A.C.E. !"),
              ("","mineur_majeur","Mineur/Majeur"),
              ("","naine_des_neiges","La naine des neiges"),
              ("","without_mi","Without mi"),
              ("","zarbi","Zarbi"),
              ("","swing","Ca swingue à Caracas")
              #("","","Dans ta F.A.C.E. 2 !")
              ] ),
          ( ("02_reprises","Reprises"),
            [ ("01_l_aventurier","","L'aventurier (perd son sang froid) / Indochine"),
              ("02_triste","","Triste / Antonio Carlos Jobim"),
              ("03_superstition","","(50 nuances de) Superstition / Stevie Wonder"),
              ("04_on_est_la","onestla","On est là (même si Macron ne veut pas) / Trad."),
              ("06_kenavo_les_bouseux","","Kenavo les bouseux / Trad. breton"),
              ("05_memories_of_tomorrow","","Memories of Tomorrow / Keith Jarrett"),
              ("07_saint_thomas_58","saint_thomas5","Saint Thomas 5/8 / Sonny Rollins"),
              ("08_le_marche_turc","marche_turc","Le marché turc / Mozart - Raoul Petite"),
              ("09_whisper_not","","Whisper not / Benny Golson"),
              ("10_mordor","","Mordor (Giorgio by Moroder) / Daft Punk"),
              ("11_i_wish_78","","I wish / Stevie Wonder"),
              ("12_apres_un_verre","","Après un verre / Gabriel Fauré (Après un rêve)"),
              ("13_risasdebolivia","risas","Risas de Bolivia / Trad. bolivien"),
              ("14_paradaija","","Paradaija / SESAM"),
              ("15_ton_rein","","Ton rein / Angèle (Ta Reine)"),
              ("16_la_sicilienne","","La Sicilienne / Gabriel Fauré"),
              ("17_doxy","","Doxy / Sonny Rollins"),
              ("18_afro_blue","","Afro Blue / Mongo Santamaría"),
              ("19_la_revolte_douce","revolte","La révolte / Sébastien Faure"),
              ("20_la_revolte_reggae","revolte","La révolte reggae / Sébastien Faure"),
              ("21_la_sicilienne","","La Sicilienne impaire / Gabriel Fauré"),
              ("22_exit_music","","Exit Music (for a PDG) / Radiohead"),
              ("23_the_final_countdown","compte_a_rebours_final","The Final Countdown / Europe"),
              ("24_oblivion","oblivion","Oblivion / Astor Piazzola"),
              ("25_le_jour_de_mon_enterrement","","Le jour de mon enterrement / Mesdames"),
              ("26_cold_song","cold_song","Chanson froide (réchauffée) / Henry Purcell")
             ])
          
          ]
 
lbd = [ ( ("01_originaux","Morceaux"), 
          [   ("06_wuhan_connect","wuhan_connect","Wuhan connect"),
              ("10_agent000","agent000","Agent 000"),  
              ("25_lemeilleurestavenir","lemeilleur","Le meilleur est à venir"),
              ("26_reverie","reverie","Rêverie"),
              ("47_samedi_matin","samedi","Samedi matin"),
              ("48_provisoire","provisoire","Provisoire"),
              ("","face","Dans ta F.A.C.E. !"),
              ("20_rdv_cascade_pisse","cascade_pisse","Rendez-vous à la cascade de la pisse")
            ] )
          ]
 
repete = [ ( ("01_originaux","Morceaux"),
             [ ("12_kronembourgvirus","kroagent", "Kronembourg-Virus Valse"),
               ("10_agent000","kroagent","Agent 000"),
               ("14_la_demoiselle_de_montreal","pour_perrine","La demoiselle de Montréal"),
               ("26_reverie","reverie","Rêverie"),
               ("29_volet","volet","Coucher de soleil (derrière un volet bloqué)"),
               ("11_rencontre_du_3eme_type","3emetype","Rencontre du 3ème type"),
               ("27_larevanchedutriton","larevanchedutriton","La revanche du Triton"),
               ("25_lemeilleurestavenir","lemeilleur","Le meilleur est à venir"),
               ("04_samba_lek","samba_lek","Samba Lek'"),
               ("../k250/kk","kk","En Chapelure (C. Cartier)"),
               ("","bouguyblouz", "Bouguy Blouz (M. Nataf)"),
               ("13_samba_calorea","samba_calorea","Samba Calor-é-a"),
               ("38_perdre_son_temps","perdresontemps","Perdre son temps"),
               ("42_ca_tourne","ca_tourne","Planète ça tourne"),
               ("43_chasse_a_l_homme","chasse_a_l_homme","Chasse à l'homme"),
               ("30_bolerototo_newage","bolero","Boléro-toto, version New-Age-Olympique"),
               ("13_samba_calorea","samba_calorea","Samba Calor-é-a"),
               ("17_c_est_pas_une_punition_han_2","punition","C'est pas une punition-han"),
               ("45_oudea_cassetoi","oudea_cassetoi","Ou-dé-a, Casse-toi-d'la !"),
               ("52_sur_un_air_de_jazz","sur_un_air_de_jazz","Sur un air de jazz"),
               ("01_prog_to_log_2000","prog","Prog-to-log 2000"),
               ("53_remous","remous","Remous"),
               ("56_5a7","5a7","Cinq à sept"),
               ("57_croisiere","croisiere","Croisière sur le Styx"),
               ("58_moerci_doerien","moerci_doerien","Moerci Doerien"),
               ("","mardi_prison","Mardi, prison de la santé")
              ]),
           ( ("02_reprises","Arrangements"),
             [
                 ("24_oblivion","oblivion","Oblivion / Astor Piazzola"),
                 ("","butnotforme","But not for me / Gershwin"),
                 ("","timeaftertime","Time after time / Styne"),
                 ("","togo","Togo (H. Texier)"),
                 ("20_la_revolte_reggae","revolte","La révolte reggae / Sébastien Faure"),
                 ("04_on_est_la","onestla","On est là (même si Macron ne veut pas) / Trad."),
                 ("23_the_final_countdown","compte_a_rebours_final","The Final Countdown / Europe"),
                 ("07_saint_thomas_58","saint_thomas5","Saint Thomas 5/8 / Sonny Rollins"),
                 ("13_risasdebolivia","risas","Risas de Bolivia / Trad. bolivien"),
                 ("","grana_od_bora","Grana od bora (trad/Bojan Z)"),
                 ("../02_reprises/26_cold_song","cold_song","Chanson froide (réchauffée) / Henry Purcell")
            ])]
             
selection2024 = [ ( ("01_originaux","2024"), 
            [ 
              ("47_samedi_matin","","Samedi matin"),
              ("48_provisoire","","Provisoire"),
              ("49_pour_manu","","Pour Manu"),
              ("50_emotion_petit_bonhomme","","Emotion du petit bonhomme"),
              ("51_satirikiki","","Satirikiki"),
              ("52_sur_un_air_de_jazz","","Sur un air de jazz"),
              ("53_remous","","Remous"),
              ("54_ponyonoke_2","","Ponyonoke dans le château sur la falaise"),
              ("55_descente_au_klezmeristan","","Descente au Klezmeristan"),
              ("56_5a7","","Cinq à sept"),
              ("../02_reprises/26_cold_song","","Chanson froide (réchauffée) / Henry Purcell")
             ])
          ]
 
